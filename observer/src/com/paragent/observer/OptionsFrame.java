...
class OptionsFrame extends Frame implements WindowListener, ActionListener,
	...
	static String[][] values = {
			...
			{ "Normal", "Reversed" }, { "Yes", "No" }, { "Yes", "No" }, };

	final int encodingIndex = 0, compressLevelIndex = 1, jpegQualityIndex = 2,
			cursorUpdatesIndex = 3, useCopyRectIndex = 4, eightBitColorsIndex = 5,
			mouseButtonIndex = 6, viewOnlyIndex = 7, shareDesktopIndex = 8;
	...
	void setColorFormat() {
		...
		if (choices[eightBitColorsIndex].getSelectedItem().equals("Full"))
			...
			eightBitColors = 3;
		else if (choices[eightBitColorsIndex].getSelectedItem()
				.equals("4 (Grey)"))
			eightBitColors = 4;
