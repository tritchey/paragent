...
class RfbProto {
	...
	final static int FramebufferUpdate = 0, SetColourMapEntries = 1, Bell = 2,
			ServerCutText = 3, rfbFileTransfer = 7;
	...
	final static int EncodingRaw = 0, EncodingCopyRect = 1, EncodingRRE = 2,
			...
			EncodingRichCursor = 0xFFFFFF11, EncodingPointerPos = 0xFFFFFF18, // marscha - PointerPos
	...
	void readVersionMsg() throws Exception {
		...
		if ((b[0] != 'R') || (b[1] != 'F') || (b[2] != 'B') || (b[3] != ' ')
				...
				|| (b[6] < '0') || (b[6] > '9') || (b[7] != '.') || (b[8] < '0')
				|| (b[8] > '9') || (b[9] < '0') || (b[9] > '9') || (b[10] < '0')
				|| (b[10] > '9') || (b[11] != '\n')) {
	...
	void startSession(String fname) throws IOException {
		...
		byte[] fbsServerInitMsg = { 32, 24, 0, 1, 0, (byte) 0xFF, 0, (byte) 0xFF,
				0, (byte) 0xFF, 16, 8, 0, 0, 0, 0 };
	...
	void readFramebufferUpdateRectHdr() throws Exception {
		...
		if (updateRectW == 0 && updateRectH == 0) {
			System.out.println("fixing rectangle");
			updateRectX = 0;
			updateRectY = 0;
		}
		...
		if (updateRectX + updateRectW > framebufferWidth
			...
			throw new Exception("Framebuffer update rectangle too large: "
					...
					+ updateRectY + "), max (" + framebufferWidth + ", "
					+ framebufferHeight + ")");
		}
	}
	...
	int writeRfbFileTransferMsgForSendFile(int contentType, int contentParam,
			long size, long length, String source) throws IOException {
		...
		while (bytesRead != -1) {
			...
			if (compressedSize > bytesRead)
				fCompress = false;
			this.writeRfbFileTransferMsg(contentType, contentParam, (fCompress ? 1
					: 0), (fCompress ? compressedSize - 1 : bytesRead - 1), null);
			// Todo: Test write error !
			os.write(fCompress ? CompressionBuffer : byteBuffer, 0,
		...
		writeRfbFileTransferMsg(fError ? rfbAbortFileTransfer : rfbEndOfFile, 0,
				0, 0, null);
		fis.close();
		return (fError ? -1 : 1);
	}
	...
	void sendFile() {
		try {
			...
			int ret = writeRfbFileTransferMsgForSendFile(rfbFilePacket, 0, 0, 0,
					sendFileSource);
	...
	void deleteRemoteFile(String text) {
		try {
			String temp = text;
			writeRfbFileTransferMsg(rfbCommand, rfbCFileDelete, 0, temp.length(),
					temp);
		} catch (IOException e) {
			System.err.println(e);
		}
	...
	void createRemoteDirectory(String text) {
		try {
			String temp = text;
			writeRfbFileTransferMsg(rfbCommand, rfbCDirCreate, 0, temp.length(),
					temp);
		} catch (IOException e) {
			System.err.println(e);
		}
	...
	void readServerDirectory(String text) {
		try {
			String temp = text;
			writeRfbFileTransferMsg(rfbDirContentRequest, rfbRDirContent, 0, temp
					.length(), temp);
		} catch (IOException e) {
			System.err.println(e);
		}
	...
	void readFTPMsgDirectoryListContent() throws IOException {
		...
		if (dwFileAttributes == 268435456 || dwFileAttributes == 369098752
				|| dwFileAttributes == 285212672 || dwFileAttributes == 271056896
				|| dwFileAttributes == 824705024 || dwFileAttributes == 807927808
				|| dwFileAttributes == 371720192 || dwFileAttributes == 369623040) {
			fileName = " [" + fileName + "]";
	...
	void readServerDriveList() {
		try {
			viewer.rfb.writeRfbFileTransferMsg(RfbProto.rfbDirContentRequest,
					RfbProto.rfbRDrivesList, 0, 0, null);
		} catch (IOException e) {
			System.err.println(e);
		}
	}
