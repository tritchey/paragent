...
public class VncViewer extends java.applet.Applet implements
	...
	public void init() {
		...
		try {
			background = this.getImage(new URL("http://www.paragent.com/i/dflysub.gif"));
	...
	public void paint(Graphics g) {
		...
		//g.drawRoundRect(centerX-100, centerY-100, centerX, centerY, 20, 20);
		
		int x = this.getWidth() - background.getWidth(this);
		int y = this.getHeight() - background.getHeight(this);
	...
	public void run() {
		...
		try {
			...
			System.out.println("doProtocolInitialisation()");
			...
			// inside.
			desktopScrollPane = new ScrollPane(ScrollPane.SCROLLBARS_ALWAYS);
	...
	public void readParameters() {
		...
		if (inAnApplet) {
			inSeparateFrame = readBoolParameter("Open New Window", false);
		}
		...
		dtPassword = readParameter("password", true);

		// "Show Controls" set to "No" disables button panel.
		showControls = readBoolParameter("Show Controls", true);

		// Do we continue showing desktop on remote disconnect?
		showOfflineDesktop = false;
		showOfflineDesktop = readBoolParameter("Show Offline Desktop", false);
	...
	boolean readBoolParameter(String name, boolean defaultValue) {
		String str = readParameter(name, false);
